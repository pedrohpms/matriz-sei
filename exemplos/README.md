...
Casos preenchidos no formato JSON da memória de cálculo (Iteração 4). Para
carregar um deles, abra o `index.html`, e no Passo 1 use **"Carregar exemplo
ou memória salva (JSON)"** e selecione o arquivo. O fluxo inteiro é
reidratado e você pode navegar pelos seis passos.
...
| [`caso-5-piso-obrigatorio.json`](caso-5-piso-obrigatorio.json) | **Piso obrigatório (ato vinculado)** por falha de segurança: score fixo de 20, pula a pontuação (`pisoAcionado: true`, `pisoJustificativa: "seguranca"`). | Piso obrigatório | 20/20 |
| [`caso-6-piso-determinacao-controle.json`](caso-6-piso-determinacao-controle.json) | Piso por **determinação de órgão de controle** (vinculante, distinta de recomendação): `pisoJustificativa: "determinacaoControle"`. | Piso obrigatório | 20/20 |
